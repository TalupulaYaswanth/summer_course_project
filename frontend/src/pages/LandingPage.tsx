  ...
  return (
    <div className="flex flex-col min-h-screen bg-darkBg text-slate-800">
      ...
      <section className="relative pt-24 pb-20 px-6 flex flex-col items-center text-center overflow-hidden border-b border-borderDark/20">
        {/* Glow meshes */}
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-accentPurple/5 rounded-full blur-[120px] pointer-events-none -z-10"></div>
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 border border-slate-200 mb-6 text-sm text-accentEmerald font-semibold shadow-sm">
          ...
          <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight leading-none mb-6 text-slate-900">
            ...
            <span className="bg-gradient-to-r from-accentPurple to-accentEmerald bg-clip-text text-transparent">
              Plain English
            </span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a
              href="#how-it-works"
              className="px-8 py-3.5 rounded-xl font-semibold text-slate-700 hover:text-slate-900 bg-slate-100 border border-slate-300 hover:bg-slate-200/50 transition-all text-base w-full sm:w-auto text-center shadow-sm"
            >
              See How It Works
            </a>
          </div>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto w-full">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-slate-900">Features Engineered for Learning</h2>
          <p className="text-slate-600 max-w-xl mx-auto">Deconstruct any algorithmic implementation with a wide array of conceptual and security analysis reports.</p>
        </div>
        ...
        >
          {features.map((feat, idx) => (
            <motion.div 
              ...
              className="glass-card p-8 rounded-2xl flex flex-col hover:border-accentPurple/40 transition-colors group cursor-pointer shadow-sm hover:shadow-md"
            >
              <div className="mb-5 p-3 rounded-xl bg-slate-50 border border-slate-200 w-fit group-hover:scale-110 transition-transform">
                {feat.icon}
              </div>
              <h3 className="text-xl font-bold mb-2 text-slate-900 group-hover:text-accentEmerald transition-colors">
                {feat.title}
              </h3>
      ...
      <section id="how-it-works" className="py-24 px-6 bg-slate-50 border-y border-borderDark/20">
        <div className="max-w-7xl mx-auto w-full">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-slate-900">How CodeExplain Works</h2>
            <p className="text-slate-600 max-w-xl mx-auto">Go from confusing syntax lines to a clean visual workbook in three quick steps.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-12 relative">
            {workflow.map((work, idx) => (
              <div key={idx} className="relative flex flex-col items-center text-center">
                <div className="text-6xl font-black text-slate-900/5 absolute -top-8 left-1/2 -translate-x-1/2 select-none z-0">
                  {work.step}
                </div>
                ...
                <h3 className="text-lg font-bold mb-2 text-slate-900">{work.title}</h3>
                <p className="text-slate-600 text-sm leading-relaxed max-w-xs">{work.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6 max-w-4xl mx-auto w-full">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-600 max-w-xl mx-auto">Got questions? We've got answers.</p>
        </div>
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => (
            <div 
              key={idx} 
              className="glass-card rounded-xl border border-borderDark/50 overflow-hidden transition-all duration-300 shadow-sm"
            >
              <button
                onClick={() => setActiveFaq(activeFaq === idx ? null : idx)}
                className="w-full px-6 py-5 flex items-center justify-between text-left font-bold text-slate-800 hover:bg-slate-50"
              >
                <span>{faq.q}</span>
                <HelpCircle className={`w-5 h-5 text-accentPurple transition-transform ${activeFaq === idx ? 'rotate-180' : ''}`} />
              </button>
              
              {activeFaq === idx && (
                <div className="px-6 pb-6 pt-1 text-slate-600 text-sm leading-relaxed border-t border-borderDark/30">
                  {faq.a}
                </div>
              )}
            </div>
      ...
      {/* CTA Footer Wrapper */}
      <footer className="mt-auto py-12 px-6 border-t border-borderDark/40 bg-white text-slate-500 text-sm">
        <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            <span className="font-bold text-slate-800 text-base">⚡ CodeExplain</span>
            <span>&copy; {new Date().getFullYear()} All rights reserved.</span>
          </div>
          <div className="flex items-center gap-6">
            <Link to="/login" className="hover:text-slate-800 transition-colors">Privacy Policy</Link>
            <Link to="/login" className="hover:text-slate-800 transition-colors">Terms of Service</Link>
            <Link to="/dashboard" className="hover:text-slate-800 transition-colors">Contact Support</Link>
          </div>
        </div>
      </footer>
    </div>
